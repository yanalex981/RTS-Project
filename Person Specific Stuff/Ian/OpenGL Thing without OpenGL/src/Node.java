 ...
 * and contains information used for pathfinding.
 * 
...
public class Node {
	 ...
	 */
	public void setHeuristic(int endX, int endY, Node parent, float cost) {
		int deltaX = Math.abs(endX - x);
		int deltaY = Math.abs(endY - y);
		this.h = (float) Math.sqrt(deltaX * deltaX + deltaY * deltaY);
		/*
		 * if(deltaY > deltaX) this.h = deltaX * Map.DIAGONAL_COST + (deltaY -
		 * deltaX) * Map.ADJACENT_COST; else this.h = deltaY * Map.DIAGONAL_COST
		 * + (deltaX - deltaY) * Map.ADJACENT_COST;
		 */

		this.parent = parent;

		if (parent != null) {
			float newG = parent.getG() + cost;
	 ...
	 */
	public void setMaxUnitRadius(float maxUnitRadius) {
