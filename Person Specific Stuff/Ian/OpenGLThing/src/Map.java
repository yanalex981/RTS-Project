...
public class Map {
	...
	public void addUnit(Unit u) {
		nodes[(int) u.getZ()][(int) u.getX()].addUnit(u);
	}
	...
	private void checkUnitPos(float x, float y, ArrayList<Unit> units,
		...
		for (int i = 0; i < units.size(); i++) {
			...
			if (Resource.magnitude2DSq(units.get(i).getCurrentPosition(),
					new Vector3f(x, 0, y)) > distBetween) {
				units.remove(i);
				i--;
			}
		}
	...
	public Unit[] getUnitsInRadius(Unit u, float radius) {
		ArrayList<Unit> units = new ArrayList<Unit>(0);
		units.addAll(getUnitsInRadius(getNode((int) u.getX(), (int) u.getZ()),
				radius + MAX_RADIUS));
		units.remove(u);
		checkUnitPos(u.getX(), u.getZ(), units, radius);
	...
	public void setUnitGridPosition(Unit u, float deltaX, float deltaY) {
		if ((int) (u.getX() - deltaX) == (int) u.getX()
				&& (int) (u.getZ() - deltaY) == (int) u.getZ())
			return;

		nodes[(int) u.getZ()][(int) u.getX()].addUnit(u);
		nodes[(int) (u.getZ() - deltaY)][(int) (u.getX() - deltaX)]
				.removeUnit(u);
	}
